...
## Cluster Expansion Follow-up

### Goal

Add free or low-cost VMs to the Talos cluster without drift out of the free tier or introducing avoidable hiccups.

### Workstreams

- [ ] Choose private access layer:
  - Tailscale
  - NetBird
- [ ] Keep Talos control-plane VIP limited to the home LAN/shared L2 case
- [ ] Use private connectivity for cloud VMs and remote access
- [ ] Make Talos configs reproducible for workers and control plane nodes
- [ ] Join new nodes with `worker.yaml` or `controlplane.yaml`
- [ ] Add usage tracking so free-tier usage does not unexpectedly drift

### Tracking

- [ ] Track instance count by provider
- [ ] Track VM shape and age
- [ ] Track CPU, network, and memory utilization
- [ ] Track Oracle idle-reclamation risk
- [ ] Track whether any paid IPs or add-ons were introduced

### Order Of Work

1. pick Tailscale or NetBird
2. establish private access to the existing cluster
3. make Talos bootstrap and join reproducible
4. add one cloud worker only
5. add usage tracking and alerts
6. test node rebuild and rejoin
7. expand only after the first node is boring
